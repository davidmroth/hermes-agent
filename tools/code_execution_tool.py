...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    description = (
        ...
        "Security model: the execute_code child process does NOT receive secrets from "
        "the parent environment. API keys, tokens, passwords, and similar env vars are "
        "intentionally scrubbed before the script starts. Do NOT use execute_code to "
        "check whether auth tokens are configured or to debug secret-dependent tool "
        "availability. Also do NOT use terminal() shell checks like `which`/`type` to "
        "test Hermes tool availability — Hermes tools are model tools, not shell commands. "
        "If a tool is in the active tool list, call it directly.\n\n"
