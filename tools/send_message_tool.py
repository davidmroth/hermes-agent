...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None):
    ...
    if media_files and not message.strip() and platform not in {Platform.SIGNAL, Platform.WEBCHAT}:
        return {
            "error": (
                f"send_message MEDIA delivery is currently only supported for telegram, discord, matrix, signal, weixin, yuanbao and webchat; "
    ...
    if media_files and platform not in {Platform.SIGNAL, Platform.WEBCHAT}:
        warning = (
            ...
            "native send_message media delivery is currently only supported for telegram, discord, matrix, signal, weixin, yuanbao and webchat"
        )
