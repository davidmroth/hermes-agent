...
class _CapturingAgent:
    ...
    def run_conversation(self, user_message, conversation_history=None, task_id=None, persist_user_message=None):
        ...
        return dict(type(self).response_payload)


def _install_fake_agent(monkeypatch):
    _CapturingAgent.last_init = None
    _CapturingAgent.last_run = None
    _CapturingAgent.response_payload = {
        "final_response": "ok",
        "messages": [],
        "api_calls": 1,
        "completed": True,
...
@pytest.mark.asyncio
async def test_run_agent_preserves_llamacpp_timings(monkeypatch, tmp_path):
    _install_fake_agent(monkeypatch)
    runner = _make_runner()
    ...
    assert result["timings"] == _CapturingAgent.response_payload["timings"]
