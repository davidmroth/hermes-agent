...
@pytest.mark.asyncio
async def test_session_hygiene_persists_final_assistant_when_new_messages_end_on_tool(
    monkeypatch, tmp_path
):
    """Persist the delivered final answer even when the turn slice ends on a tool.

    Regression guard for reconciled sessions whose JSONL transcript was left
    ending on tool output despite a successful gateway response.
    """
    fake_dotenv = types.ModuleType("dotenv")
    fake_dotenv.load_dotenv = lambda *args, **kwargs: None
    monkeypatch.setitem(sys.modules, "dotenv", fake_dotenv)
    ...
    assert assistant_entries, "Expected a persisted assistant transcript entry"
